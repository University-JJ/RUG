...
for i= 1: numImages
  im=images(:,:, i); %We've selected the image at location i
  for j= 1 : numFilters
    filter = W(:,:,j); %We've selected the filter
    filter = rot90(rot90(filter)); %We've flipped the matrix
    result = conv2(im, filter, 'valid'); %We've used the convolution built-in function
    bias = b(j,1); %We've stores the bias
    activation = sigmoid(bias+result); %We've calculated the activations
    convolvedFeatures(:, :, j, i) = activation; %We've stored the activation in convolvedFeatures    
  end
end
